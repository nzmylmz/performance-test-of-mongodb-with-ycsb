...
public class CoreWorkload extends Workload
{
  ...
  public static final String HOTSPOT_OPN_FRACTION_DEFAULT = "0.8";
	...
	HashMap<String, String> buildValues() {
 		HashMap<String,String> values=new HashMap<String,String>();
                fieldcount=6;
		String alanAdi[]={"STATION","NAME","DATE","TAVG","TMAX","TMIN"};
                Random rnd=new Random();
                int year = rnd.nextInt(2)+2017;
                int month = rnd.nextInt(12)+1;
                YearMonth yearMonthObject = YearMonth.of(year, month);
                int daysInMonth = yearMonthObject.lengthOfMonth();           
                int day = rnd.nextInt(daysInMonth)+1;
				...
				String TMIN=null;
				if(a>b){
					TMAX= String.valueOf(a); 
					TMIN= String.valueOf(b); 	
				}
				else{
					 TMIN= String.valueOf(a); 
					 TMAX= String.valueOf(b); 
				}
                String TAVG = String.valueOf((a+b)/2);
				
		String data[]={"TUM00017064","ISTANBUL BOLGE KARTAL, TU",tarih,TAVG,TMAX,TMIN};
 		for (int i=0; i<fieldcount; i++)
 		{
 			//String fieldkey=alanAdi[i];
			values.put(alanAdi[i],data[i]);		
			//ByteIterator data= new RandomByteIterator(fieldlengthgenerator.nextInt());
 			//values.put(fieldkey,data);
                }
		return values;
	}
	HashMap<String, String> buildUpdate() {
		//update a random field
		fieldcount=6;
		String alanAdi[]={"STATION","NAME","DATE","TAVG","TMAX","TMIN"};
		HashMap<String, String> values=new HashMap<String,String>();
		Random r=new Random();
		int a=r.nextInt(fieldcount);
		String fieldname=alanAdi[a];
		String data[]={"TUM00035890","İZMİR BÖLGE ÖDEMİŞ, TUR","25-03-2018","50","99","00"};
                //ByteIterator data = new RandomByteIterator(fieldlengthgenerator.nextInt());
		values.put(fieldname,data[a]);
		return values;
	}
